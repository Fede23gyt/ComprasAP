            ...
            <div v-else-if="step === 'create_budget'">
              ...
              <form @submit.prevent="submit">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <!-- Tipo de Compra -->
                  <div>
                    <InputLabel for="tipo_compra_id" value="Tipo de Compra *" class="text-gray-900 dark:text-gray-100" />
                    <select
                      id="tipo_compra_id"
                    ...
                    <!-- Alerta para licitaciones -->
                    <div v-if="esLicitacion" class="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                      <div class="flex">
                        ...
                        <div class="ml-3">
                          ...
                          <div class="mt-2 text-sm text-yellow-700">
                            <p>Los precios en procesos de licitación son fijos y no pueden modificarse. Se utilizarán los precios definidos en las notas de pedido.</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <!-- Número de Llamado -->
                  <div>
                    <InputLabel for="nro_llamado" value="Número de Llamado *" class="text-gray-900 dark:text-gray-100" />
                    <TextInput
                      id="nro_llamado"
                      type="number"
                  ...
                  <div>
                    <InputLabel for="domicilio_entrega" value="Domicilio de Entrega" class="text-gray-900 dark:text-gray-100" />
                    <TextInput
                  ...
                  <!-- Lugar de Entrega -->
                  <div>
                    <InputLabel for="lugar_entrega" value="Lugar de Entrega" class="text-gray-900 dark:text-gray-100" />
                    <TextInput
                      id="lugar_entrega"
                      type="text"
                      class="mt-1 block w-full"
                  ...
                  <!-- Dependencia -->
                  <div>
                    <InputLabel for="dependencia" value="Dependencia" class="text-gray-900 dark:text-gray-100" />
                    <TextInput
                      id="dependencia"
                      type="text"
                      class="mt-1 block w-full"
                  ...
                  <!-- Forma de Pago -->
                  <div>
                    <InputLabel for="forma_pago" value="Forma de Pago" class="text-gray-900 dark:text-gray-100" />
                    <TextInput
                      id="forma_pago"
                      type="text"
                      class="mt-1 block w-full"
                  ...
                  <!-- Plazo de Pago -->
                  <div>
                    <InputLabel for="plazo_pago" value="Plazo de Pago" class="text-gray-900 dark:text-gray-100" />
                    <TextInput
                      id="plazo_pago"
                      type="text"
                      class="mt-1 block w-full"
                  ...
                  <!-- Plazo de Entrega -->
                  <div>
                    <InputLabel for="plazo_entrega" value="Plazo de Entrega (días)" class="text-gray-900 dark:text-gray-100" />
                    <TextInput
                      id="plazo_entrega"
                      type="number"
                      class="mt-1 block w-1/4"
                  ...
                  <div>
                    <InputLabel for="validez_oferta" value="Validez de la Oferta (días)" class="text-gray-900 dark:text-gray-100" />
                    <TextInput
                      id="validez_oferta"
                      type="number"
                      class="mt-1 block w-1/4"
                ...
                <!-- Observaciones y Comentarios en la misma línea -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <!-- Observaciones -->
                  <div>
                    <InputLabel for="observaciones" value="Observaciones" class="text-gray-900 dark:text-gray-100" />
                    <textarea
                      id="observaciones"
                      class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                  ...
                  <!-- Comentarios -->
                  <div>
                    <InputLabel for="comentario" value="Comentarios" class="text-gray-900 dark:text-gray-100" />
                    <textarea
                      id="comentario"
                      class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                        ...
                        <tr v-for="(detalle, index) in form.detalles" :key="index">
                          ...
                          <td class="px-4 py-3">
                            <TextInput
                              ...
                              :readonly="esLicitacion"
                              :class="[
                                'w-full',
                                esLicitacion ? 'bg-gray-100 dark:bg-gray-700 cursor-not-allowed' : ''
                              ]"
                              @input="calcularSubtotal(index)"
                            />
                            <p v-if="esLicitacion" class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                              Precio fijo en licitación
                            </p>
                          </td>
                          <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <!-- Selector de Memo -->
                  <div>
                    <InputLabel for="memo_id" value="Memo a incluir" class="text-gray-900 dark:text-gray-100" />
                    <select
                      id="memo_id"
                      class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                      v-model="selectedMemoId"
                    >
                      <option value="">Seleccionar memo (opcional)</option>
                      <option v-for="memo in props.memos" :key="memo.id" :value="memo.id">
                        {{ memo.nombre }}
                      </option>
                    </select>
                  </div>
                  ...
                  <div v-if="selectedMemo">
                    <InputLabel value="Contenido del memo" class="text-gray-900 dark:text-gray-100" />
                    <div class="mt-1 block w-full border border-gray-300 dark:border-gray-700 dark:bg-gray-700 rounded-md shadow-sm p-3">
                      <div class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ selectedMemo.descripcion }}</div>
                    </div>
                  </div>
                </div>
...
const selectedMemo = computed(() => {
  if (!selectedMemoId.value) return null
  return props.memos.find(memo => memo.id == selectedMemoId.value)
})
...
// Verificar si es una licitación basado en el tipo de compra
const esLicitacion = computed(() => {
  if (!form.tipo_compra_id) return false
  const tipoCompra = props.tiposCompra?.find(tipo => tipo.id === form.tipo_compra_id)
  return tipoCompra?.descripcion?.toLowerCase().includes('licitación') ||
         tipoCompra?.descripcion?.toLowerCase().includes('licitacion') ||
         tipoCompra?.descripcion?.toLowerCase().includes('concurso')
})
