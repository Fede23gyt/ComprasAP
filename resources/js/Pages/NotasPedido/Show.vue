  ...
  <AppLayout :breadcrumbs="breadcrumbs">
      ...
      <div class="max-w-7xl mx-auto px-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-3">
            ...
            >
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-2a2 2 0 00-2-2H9a2 2 0 00-2 2v2a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/>
              </svg>
    ...
    <section class="max-w-7xl mx-auto px-4 py-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
            <div class="flex items-center space-x-2">
              ...
              <div class="min-w-0 flex-1">
                <div class="text-xs font-medium text-gray-900 dark:text-white">Oficina</div>
                <div class="text-xs text-gray-600 dark:text-gray-300 truncate">{{ nota.oficina.nombre }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
            <div class="flex items-center space-x-2">
              ...
              <div class="min-w-0 flex-1">
                <div class="text-xs font-medium text-gray-900 dark:text-white">Tipo</div>
                <div class="text-xs text-gray-600 dark:text-gray-300 truncate">{{ nota.tipo_nota.descripcion }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
            <div class="flex items-center space-x-2">
              ...
              <div class="min-w-0 flex-1">
                <div class="text-xs font-medium text-gray-900 dark:text-white">Solicitante</div>
                <div class="text-xs text-gray-600 dark:text-gray-300 truncate">{{ nota.usuario.name }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
            <div class="flex items-center space-x-2">
              <div class="p-1.5 rounded-lg" :class="nota.estado === 0 ? 'bg-yellow-100 dark:bg-yellow-900/20' : nota.estado === 1 ? 'bg-green-100 dark:bg-green-900/20' : 'bg-red-100 dark:bg-red-900/20'">
                <svg class="w-4 h-4" :class="nota.estado === 0 ? 'text-yellow-600 dark:text-yellow-400' : nota.estado === 1 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path v-if="nota.estado === 0" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                  <path v-else-if="nota.estado === 1" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                  <path v-else stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
              </div>
              <div class="min-w-0 flex-1">
                <div class="text-xs font-medium text-gray-900 dark:text-white">Estado</div>
                <div class="text-xs truncate" :class="getEstadoClasses(nota.estado).replace('bg-', 'text-').replace('100', '600').replace('dark:bg-', 'dark:text-').replace('900', '400')">
                  {{ getEstadoTexto(nota.estado) }}
                </div>
              </div>
            </div>
        ...
        <div v-if="nota.expediente || (nota.estado === 1 && nota.confirmado_por)" class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">

          <!-- Expediente (si existe) -->
          <div v-if="nota.expediente" class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
            <div class="flex items-center space-x-2">
              <div class="p-1.5 bg-yellow-100 dark:bg-yellow-900/20 rounded-lg">
                <svg class="w-4 h-4 text-yellow-600 dark:text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
              </div>
              <div class="min-w-0 flex-1">
                <div class="text-xs font-medium text-gray-900 dark:text-white">Expediente</div>
                <div class="text-xs text-gray-600 dark:text-gray-300 truncate">{{ nota.expediente }}</div>
              </div>
            </div>
          </div>

          <!-- Información de confirmación (si está confirmada) -->
          <div v-if="nota.estado === 1 && nota.confirmado_por" class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
            <div class="flex items-center space-x-2">
              <div class="p-1.5 bg-green-100 dark:bg-green-900/20 rounded-lg">
                <svg class="w-4 h-4 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="min-w-0 flex-1">
                <div class="text-xs font-medium text-gray-900 dark:text-white">Confirmada por</div>
                <div class="text-xs text-gray-600 dark:text-gray-300 truncate">{{ nota.confirmado_por.name }}</div>
...
const imprimirPDF = () => {
  ...
  window.open(route('notas-pedido.pdf', props.nota.id), '_blank')
}
</script>