...
const canAccessConfig = computed(() => {
  ...
  console.log('DEBUG: Usuario completo:', user.value);
  ...
  if (user.value.roles && user.value.roles.length > 0) {
    roleName = user.value.roles[0].name;
    console.log('DEBUG: Rol encontrado via Spatie:', roleName);
  }
  // 2. Rol tradicional como objeto
  else if (user.value.role && typeof user.value.role === 'object') {
    roleName = user.value.role.name;
    console.log('DEBUG: Rol encontrado via tradicional (objeto):', roleName);
  }
  // 3. Rol tradicional como string directo
  else if (user.value.role && typeof user.value.role === 'string') {
    roleName = user.value.role;
    console.log('DEBUG: Rol encontrado via tradicional (string):', roleName);
  }
  
  if (!roleName) {
    console.log('DEBUG: No se encontró rol');
    return false;
  }
  
  const hasAccess = ['administrador', 'secretario', 'director'].includes(roleName);
  console.log('DEBUG: Rol:', roleName, 'Tiene acceso:', hasAccess);
...
const canAccessFullFeatures = computed(() => {
  ...
  return ['administrador', 'secretario', 'director'].includes(roleName);
});

// Definir el menú de navegación
const menu = computed(() => [
  // Solo mostrar administración a admins, secretarios y directores
  ...(canAccessConfig.value ? [{
    ...
    items: [
      ...
      { label: 'Reportes', route: '/admin/reports', icon: ChartBarIcon },
    ],
  }] : []),
  {
    key: 'configuracion',
    ...
    isSubmenu: true,
    items: [
      { label: 'Oficinas', route: '/nomencladores/oficinas', icon: BuildingOfficeIcon },
      { label: 'Tipos de Nota', route: '/nomencladores/tipos-nota', icon: DocumentTextIcon },
      { label: 'Tipos de Compra', route: '/nomencladores/tipos-compra', icon: ShoppingCartIcon },
    ],
  },
  {
    key: 'notas-pedido',
    categoria: 'Notas de Pedido',
    icon: ClipboardDocumentListIcon,
    isSubmenu: true,
    items: [
      { label: 'Mis Notas', route: '/notas-pedido', icon: DocumentTextIcon },
      ...
      ...(canAccessFullFeatures.value || (user.value?.roles && user.value.roles[0]?.name === 'operador') ? [
        { label: 'Por Confirmar', route: '/notas-pedido/confirmar', icon: CheckCircleIcon }
      ] : []),
    ],
  },
      ...
      <div class="flex items-center space-x-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center space-x-2 mt-1">
            <!-- Rol del usuario -->
            <span v-if="user.roles && user.roles.length > 0" class="text-xs bg-orange-600 text-white px-2 py-0.5 rounded-full">
